...
# Hi - the inverse of the Hessian matrix at the minimum 
  ...
  while (iter < maxit) {
    if (max(abs(gradient)) < (abs(f0)+fscale)*tol){# if converged
      ...
      while ((func(theta + Delta)[1] > f0[1]) | # check if we reduce objective
        ...
        } else {# give warning If the step fails to reduce the objective despite trying max.half step halvings
